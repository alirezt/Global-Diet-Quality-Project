...
d <- read.spss("Rwarehouse/06-05-2023/output/Individual observations.sav", use.value.labels=F, to.data.frame=TRUE, stringsAsFactors=FALSE)
...
write.csv(a, "Rwarehouse/06-05-2023/output/GWP variables.csv", row.names=FALSE)
...
for(k in unique(d$COUNTRYNEW)){
  curcountry <- k
   ...
   curdat <- d[d$COUNTRYNEW == curcountry, ] #A:curdat is a dataframe constructed for each country so it only comprise of a one country and then
                                             # in next cycle of k, next country df is constructed. 
    # Overall
    # Create weighted object #A: WP12259 is PSU or sampling stage and WP12258A is Sampling Stratification 2
    ...
    for(i in c("FGDS", "GDR_H_score", "GDR_L_score", "GDR_score", "All5_score", "DQQ_score", "WB")){
      ...
      # all
        ...
        result1 <- rbind(result1, c(unique(as.character(curdat$WBIG[curdat$COUNTRYNEW == curcountry])), 
                                    ...
                                    curcountry, 
                                    unique(as.character(curdat$COUNTRY_ISO3[curdat$COUNTRYNEW == curcountry])), 
                                    "All", 
                                    i, 
                                    attributes(d)$variable.labels[i], round(x, digits = 2), 
        ...
        x <- svymean(~d_m[, i],  d_m_w, na.rm = TRUE, method = "as", df=degf(d_m_w))
             ...
             result1 <- rbind(result1, c(unique(as.character(curdat$WBIG[curdat$COUNTRYNEW == curcountry])), 
                                        ...
                                        "Male", 
                                        i, 
                                        attributes(d)$variable.labels[i], 
                                        round(x, digits = 2), 
                                        round(x_LCI, digits = 2), 
 ...
 ######################
        ...
        for(i in c("FV_400", "Fib_25",  "nutseeds_score", "SF_10", "Sug_10", "LessSug_10", "otherfruit_fv_fib", "otherfruit_MDD", "OneVegetable", "OneFruit", "ForV", "NoFV", "OneProtein", "All5",
            ...
            result2 <- rbind(result2, c(unique(as.character(curdat$WBIG[curdat$COUNTRYNEW == curcountry])), 
                                        ...
                                        round(x_UCI, digits = 4), NA, NA, NA, NA))
