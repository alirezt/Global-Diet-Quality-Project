...
figure3 <- function(df,dfline, Country_name){
  Country_name <- rlang::parse_expr(quo_name(enquo(Country_name)))
  df %>%
    filter(Country == Country_name &
             Variable_label %in% c("Soft drinks (soda)",
                         "Sweet foods",
                         "Deep fried food",
    ...
    geom_text(aes(label = paste0(mean,"%"), y = mean + 1.9, color = Subgroup), 
              size = 4.5, vjust = rep(c(0.5, 0.7, 0.5, 0.3), 5), hjust = 0, facefont = "bold", family = "Whitney semibold",   position = position_dodge(-.9), show.legend = FALSE) +
    ...
    scale_x_discrete(limits = c("Soft drinks (soda)",
                                "Sweet foods",
                                "Deep fried food",
                                "Salty snacks, instant noodles, or fast food",
                                "Processed meat"),
                     
                     labels = c("Processed meat" = "Processed \nmeats ",
                                "Deep fried food" = "Deep fried food ",
                                "Salty snacks, instant noodles, or fast food"= "Salty snacks, \ninstant noodles, \n or fast food ", 
                                "Sweet foods" = "Sweet foods ",
                                "Soft drinks (soda)" = "           Soft drinks "), expand = c(.11,.11)) +
...
figure3 <- figure3(df = df1, dfline = linesdata ,  Country_name = "'India'")
...
d <- df %>%
  ...
  filter(Country == "India" &
           Variable_label %in% c("GDR-Limit Score (0-9)"))
...
GDR_Limit_max <- data.frame("Country" = c("India"),
                      "Subgroup"= c("All"), 
                      "Variable_label" = c("GDR-Limit Score (0-9)"),
                      "mean_pre" = 9 - GDR_limitscore$mean_pre,
                      "stack" = c("max"))

# Food group diversity score;

GDR_limit <- rbind(GDR_limitscore, GDR_Limit_max)


colorPanel = c("#F0CDC9", "#C23626")


GDRL <- ggplot(GDR_limit, aes(x = Variable_label, y = mean_pre, fill = stack)) + 
  ...
  labs( x= " ",
        y = " ") +  
  annotate("text", x = 1.1, y = d$mean_pre[1] + .15, label = round(d$mean_pre[1], digits = 1),  family = "whitney semibold",  color = "#C23626", fontface = "bold", size = 4.5, hjust = 0) +
  annotate("text", x = 4, y = 0, label = "Global dietary recommendations-limit (#/9)\n", size = 5, family =  "whitney semibold", fontface = "bold", hjust = 0, lineheight = .95) +
...
Brief <- plot_grid(figure3, GDRL, rel_heights = c(1, 0.1), nrow = 2, ncol = 1)

theme(plot.margin = unit(c(0, 0, 0, 0), "cm"))


ggsave(file = "graphs/India_Figure 3.pdf", width = 12.5,  height = 15, units ='cm', device = cairo_pdf, 
       Brief)