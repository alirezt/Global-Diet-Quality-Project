...
#  Read dataset:
df0 = read_sav("Rwarehouse/06-05-2023/Diet_Quality_2021_042722.sav") 
write_csv(df0, "Rwarehouse/06-05-2023/output/df0.csv")
...
# Create well being variable (good life if on 5th rung or higher on a 10 step ladder)
...
df0$age <- ifelse(df0$age == 100, NA, df0$age) #118 individuals 
...
write_csv(column_lookup, "Rwarehouse/06-05-2023/output/column_lookup.csv")

# create a function to replace the old variables from questions with new variables names created 
...
glimpse(df2)
 ...
 missingcols <- allcols[!allcols %in% colnames(df2)] #"DQ10c" not existed in the initial dataset 
...
if(length(missingcols) >0){
 ...
 falseifNA <- function(x){
   ifelse(is.na(x), FALSE, x) # replace NAs with zeros 
 }
...
is.na(c(1,2,NA,NA,0,0.5)) # FALSE FALSE  TRUE  TRUE FALSE FALSE
falseifNA(c(1,2,NA,NA,0,0.5)) # 1.0 2.0 0.0 0.0 0.0 0.5
ifelse2(c(1,2,NA,NA,0,0.5), 1, 0) # 1 1 0 0 0 1
...
write_csv(df2, "Rwarehouse/06-05-2023/output/df2.csv") 
#********************************##
...
# 1. Staple foods made from Grains, whole grains,  white roots and tubers
...
# Legumes	
df$legume_fiber = ifelse2(df$DQ4 ==1, 2, 0) # why 2?? 
...
write_csv(df.done, "Rwarehouse/06-05-2023/output/Individual observations.csv")

write_sav(df.done, "Rwarehouse/06-05-2023/output/Individual observations.sav")
