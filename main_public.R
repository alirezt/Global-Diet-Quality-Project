...
indpri <- readxl::read_excel(
  path = "Data/raw/Indicator hierarchy and meta-data for GDQ project website_Apr2024.xlsx", 
  col_names = T
)

indpri %<>%
  select(`Indicator number`, `Indicator name`)

indpri$`Indicator name` <- str_trim(string = indpri$`Indicator name`, side = "right")  
