...
# Paste SAV file in your working directory
...
## 2.2. Replacing India, Israel and Malaysia DQQ ----
...
## 2.3. Replacing initial NA with 0 ----
d %<>%
  mutate(
...
#### 3.16.1. at least one vegetable or fruit   ----
d %<>% mutate(
...
#### 3.16.2. Salty snacks, instant noodles, or fast food  ----
d %<>% mutate(
...
result1 <- setNames(data.frame(matrix(ncol = 11, nrow = 1)), c("World_bank_income_group", "Region", "Country", "ISO3", "Subgroup", "Variable", "Start_Date", "End_Date", "Mean_prevalence", "Lower_95_CI", "Upper_95_CI"))
result2 <- setNames(data.frame(matrix(ncol = 11, nrow = 1)), c("World_bank_income_group", "Region", "Country", "ISO3", "Subgroup", "Variable", "Start_Date", "End_Date", "Mean_prevalence", "Lower_95_CI", "Upper_95_CI"))
d <- data.frame(d)
...
results <- results %>%
  ...
  relocate(Start_Date, .after = Upper_95_CI) %>%
  relocate(End_Date, .after = Start_Date) %>%
  rename(Indicator = Variable)

# Adding indicator number 
indpri <- tibble(
  `Indicator number` = c(
...
setdiff(results$Indicator, indpri$`Indicator name`) # should be zero
...
# Adding n for each country-subgroup combination 
d_a <- d %>%
  group_by(Country) %>%
  summarise(n = n()) %>%
  mutate(Subgroup = "All")
...
results %<>%
  ...
  relocate(n, .after = Upper_95_CI) 

write.csv(results, "DQQ_GWP_2021-2022_2023_Public_10April2024.csv", row.names = F)
