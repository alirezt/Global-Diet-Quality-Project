...
glimpse(d)
d <- d[ , c("STRATA", "FieldDate", "PSU", "CaseID", "WPID", "Weight", "FieldDate", "Country", "Gender", 
...
d$zvegfr <- ifelse((rowSums(d[c("DQQ5","DQQ6_1","DQQ6_2", "DQQ7_1", "DQQ7_2", "DQQ7_3", "DQQ8", "DQQ9", "DQQ10_1", "DQQ10_2")] == 1, na.rm=TRUE) > 0) == TRUE, 0, 1)
...
d$Residence <- ifelse(is.na(d$Residence),
                       ifelse(d$WP7572 == 1, "Urban",
                              ifelse(d$WP7572 == 2, "Rural", NA)), d$Residence) # not helpful. remaining NAs 19,571 
...
head(results)
glimpse(results)
write_csv(results, "Data/Output/CSV/results2022.csv")
