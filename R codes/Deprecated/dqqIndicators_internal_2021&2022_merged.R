#########     DQQ-based indicators (merged version)    #########
##############           (Internal format)        ##############
...
# 2. Data preparation ----
d = read.spss("Data/Input/Sav/2022/Diet_Quality_032023_INTERNAL.sav", 
              use.value.labels = F, 
              to.data.frame = TRUE, 
...
d$FieldDate <- format(as.Date(d$FieldDate/86400, origin = "1582-10-14"), "%B%Y")
...
result1 <- setNames(data.frame(matrix(ncol = 14, nrow = 1)), c("Income_group", "Region", "Country", "ISO3", "Subgroup", "Variable", "Date", "Mean_Prevalence", "LCI", "UCI", "Difference", "Diff_LCI", "Diff_UCI", "Diff_p"))
result2 <- setNames(data.frame(matrix(ncol = 14, nrow = 1)), c("Income_group", "Region", "Country", "ISO3", "Subgroup", "Variable", "Date", "Mean_Prevalence", "LCI", "UCI", "Difference", "Diff_LCI", "Diff_UCI", "Diff_p"))
...
#    curcountry <- k
#    yr <- j
#    curdat <- d[d$Country == curcountry & d$YEAR_WAVE == yr, ]
    ...
    for(i in c("fgds", "ncdp", "ncdr", "gdr")){
      ...
      result1 <- rbind(result1, c(unique(as.character(curdat$Income.classification[curdat$Country == curcountry])), 
                                  ...
                                  "All", 
                                  i,
                                  unique(as.character(curdat$FieldDate[curdat$Country == curcountry])),
                                  round(x, digits = 2), 
...
head(results)
glimpse(results)
write_csv(resultst, "Data/Output/CSV/results_internal_merged_WithDate.csv")

# 6. Disaggregation ----
table(cut(d$Age, breaks = c(15,25,50,101), right = FALSE))
sum(table(cut(d$Age, breaks = c(15,25,50,101), right = FALSE)))

d <- d %>% 
  mutate(AgeL = cut(Age, breaks = c(15,25,50,101), right = FALSE))

#Residence, AgeL
d_dis_fm <- d %>% 
  select(Country, Gender, CaseID) %>%
  group_by(Country, Gender) %>%
  summarise(n = n()) %>%
  pivot_wider(names_from = c(Gender), values_from = n) 

d_dis_r <- d %>% 
  select(Country, Residence, CaseID) %>%
  group_by(Country, Residence) %>%
  summarise(n = n()) %>% 
  pivot_wider(names_from = c(Residence), values_from = n) 

d_dis_age <- d %>% 
  select(Country, AgeL, CaseID) %>%
  group_by(Country, AgeL) %>%
  summarise(n = n()) %>% 
  pivot_wider(names_from = c(AgeL), values_from = n) 

d_dis <- d_dis_fm %>%
  left_join(d_dis_r) %>%
  left_join(d_dis_age) %>%
  mutate(total = Male + Female) %>%
  rename("15-24 years" = `[15,25)`,
         "25-49 years" = `[25,50)`,
         "50+ years" = `[50,101)`) %>%
  relocate(total, .before = Male) %>%
  relocate(`NA`, .after = `50+ years`)

write_csv(d_dis, "Data/Output/CSV/temp/d_dis.csv")
