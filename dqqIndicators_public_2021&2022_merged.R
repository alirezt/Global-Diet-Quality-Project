######## DQQ-based Indicators for 2021 and 2022 (merged version) ######
...
# 2. Data preparation ----
d= read.spss("Data/Input/Sav/2022/Diet_Quality_032023_PUBLIC.sav", 
             use.value.labels=F, to.data.frame=TRUE, stringsAsFactors=FALSE)
d$FieldDate <- format(as.Date(d$FieldDate/86400, origin = "1582-10-14"), "%m")

d %>%
  group_by(FieldDate, Country) %>%
  count() %>%
  arrange(Country)%>%
  print(n = 60)

write_csv(d, "Data/Output/CSV/test.csv")

d <- d[ , c("STRATA", "PSU", "CaseID", "Weight", 
            "Country", "COUNTRY_ISO3", "YEAR", "Gender", "Age",
...
## 2.1 India and Israel DQQ ----
# Replace DQQ of India and Israel to their corresponding main DQQ columns
...
## 2.2 Replacing 'DK' and 'Refused' answers with NA and 0 ----
d <- d %>%
...
### 3.20. Percent consuming each food group   ----
#### 3.20.a. at least one vegetable or fruit   ----
...
#### 3.20.b. Salty snacks, instant noodles, or fast food  ----
...
result1 <- setNames(data.frame(matrix(ncol = 9, nrow = 1)), c("Income_group", "Region", "Country", "ISO3", "Subgroup", "Variable", "Mean_Prevalence", "LCI", "UCI"))
result2 <- setNames(data.frame(matrix(ncol = 9, nrow = 1)), c("Income_group", "Region", "Country", "ISO3", "Subgroup", "Variable", "Mean_Prevalence", "LCI", "UCI"))
...
write_csv(results, "Data/Output/CSV/results_public_merged.csv")
