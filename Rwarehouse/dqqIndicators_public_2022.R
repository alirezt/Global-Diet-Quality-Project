...
# 2. Data preparation ----
d= read.spss("Data/Input/Sav/2022/Diet_Quality_032023_PUBLIC.sav", use.value.labels=F, to.data.frame=TRUE, stringsAsFactors=FALSE)
d <- d[ , c("STRATA", "PSU", "YEAR", "CaseID", "Weight", "FieldDate", "Country", "Gender", 
            "Age", "Education", "IncomeQuintiles", "COUNTRY_ISO3",
            "Urbanicity", "DEGURBA_2021_F2F", "DEGURBA_2021_PHONE", "DEGURBA_2022", 
...
d <- d %>%
  mutate(across(.cols = starts_with("DQQ"), .fns = ~ replace(.x, .x == 8 | .x == 9, NA)))
...
# 4. Data preparation (new columns and encoding factor variables) ----
## 4.1 New columns ----
# Removing extra space in countries names
d$Country <- str_trim(d$Country, side = "right")

# Add ISO, Income, and Region
d$Region <- ifelse(d$Country == "Egypt" | d$Country == "Israel" | d$Country == "Jordan" | d$Country == "Lebanon" | d$Country == "Morocco", "MENA",
                   ifelse(d$Country == "Greece" | d$Country == "Kazakhstan" | d$Country == "Russia" | d$Country == "Tajikistan" | d$Country == "Turkey" | d$Country == "Albania" | d$Country == "Armenia" | d$Country == "Azerbaijan" | d$Country == "Kyrgyzstan" | d$Country == "Uzbekistan", "ECA",                                                                   
                          ifelse(d$Country == "Bangladesh" | d$Country == "Cambodia" | d$Country == "China" | d$Country == "India" | d$Country == "Indonesia" | d$Country == "Laos" | d$Country == "Nepal" | d$Country == "Pakistan" | d$Country == "Philippines" | d$Country == "Sri Lanka" | d$Country == "Vietnam" | d$Country == "Afghanistan" | d$Country == "Palestine" | d$Country == "Yemen", "AP",                             
...
result1 <- setNames(data.frame(matrix(ncol = 11, nrow = 1)), c("Income_group", "Region", "Country", "ISO3", "Subgroup", "Variable", "Year", "Mean_Prevalence", "LCI", "UCI"))
result2 <- setNames(data.frame(matrix(ncol = 11, nrow = 1)), c("Income_group", "Region", "Country", "ISO3", "Subgroup", "Variable", "Year", "Mean_Prevalence", "LCI", "UCI"))
...
head(results)
glimpse(results)
write_csv(results, "Data/Output/CSV/results_public.csv")
