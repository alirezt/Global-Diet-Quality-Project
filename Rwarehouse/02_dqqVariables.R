...
d$all5a <- ifelse((rowSums(d[c("DQQ5", "DQQ6.1", "DQQ6.2", "DQQ7.1", "DQQ7.2", "DQQ7.3")] == 1, na.rm=TRUE) > 0) == TRUE, 1, 0)
#### 2.b.	At least one fruit  ----
d$all5b <- ifelse((rowSums(d[c("DQQ8","DQQ9", "DQQ10.1", "DQQ10.2")] == 1, na.rm=TRUE) > 0) == TRUE, 1, 0)
...
d$zvegfr <- ifelse((rowSums(d[c("DQQ5","DQQ6.1","DQQ6.2", "DQQ7.1", "DQQ7.2", "DQQ7.3", "DQQ8", "DQQ9", "DQQ10.1", "DQQ10.2")] == 1, na.rm=TRUE) > 0) == TRUE, 1, 0)
...
d$vegfr <- ifelse((rowSums(d[c("DQQ5","DQQ6.1","DQQ6.2", "DQQ7.1", "DQQ7.2", "DQQ7.3", "DQQ8", "DQQ9", "DQQ10.1","DQQ10.2")] == 1, na.rm=TRUE) > 0) == TRUE, 1, 0)
...
d$dveg <- ifelse((rowSums(d[c("DQQ6.1","DQQ6.2")] == 1, na.rm=TRUE) > 0) == TRUE, 1, 0)

#### 23. Animal-based food ----
d$anml <- ifelse((rowSums(d[c("DQQ17","DQQ18", "DQQ19", "DQQ20")] == 1, na.rm=TRUE) > 0) == TRUE, 1, 0)

#### 24. Other fruits ----
d$ofr <- ifelse((rowSums(d[c("DQQ10.1","DQQ10.2")] == 1, na.rm=TRUE) > 0) == TRUE, 1, 0)

#### 25. Other vegetables ----
d$oveg <- ifelse((rowSums(d[c("DQQ7.1","DQQ7.2", "DQQ7.3")] == 1, na.rm=TRUE) > 0) == TRUE, 1, 0)
...
dpc <- d %>% 
  ...
  summarise(
    ...
    ofr_pc   = round(mean(rowSums(across(DQQ10.1:DQQ10.2) == 1, na.rm=TRUE) >= 1)*100, digits = 2), #Other fruits
...
# 8. Subgroup data set ----
## 8.1 Vector names ----
cntryNames <- c("Bangladesh", "Benin", "Bolivia", "Burkina Faso", "Cambodia", "Cameroon", "Chile", "China",
                "Colombia", "Ecuador", "Egypt", "Gabon", "Ghana", "Greece", "India", "Indonesia", "Israel", "Jordan",
                "Kazakhstan", "Kenya", "Laos", "Lebanon", "Mexico", "Morocco", "Mozambique", "Nepal", "Nicaragua",
                "Nigeria", "Pakistan", "Philippines", "Russia", "Senegal", "Sierra Leone", "South Africa", "Sri Lanka",
                "Tajikistan", "Tanzania", "Turkey", "Uganda", "United States", "Vietnam") 

cntryISO3 <- c("BGD", "BEN", "BOL", "BFA", "KHM", "CMR", "CHL", "CHN", "COL", "ECU", "EGY", "GAB", 
               "GHA", "GRC", "IND", "IDN", "ISR", "JOR", "KAZ", "KEN", "LAO", "LBN", "MEX", "MAR",
               "MOZ", "NPL", "NIC", "NGA", "PAK", "PHL", "RUS", "SEN", "SLE", "ZAF", "LKA", "TJK",
               "TZA", "TUR", "UGA", "USA", "VNM")

cntryRegion <- c("AP", "SSA", "AM", "SSA", "AP",  "SSA", "AM",  "AP", "AM", "AM", "MENA",
                 "SSA", "SSA", "ECA", "AP", "AP", "MENA", "MENA", "ECA",  "SSA",  "AP", "MENA",
                 "AM",  "MENA", "SSA",  "AP", "AM", "SSA",  "AP", "AP", "ECA",  "SSA",  "SSA",
                 "SSA", "AP", "ECA",  "SSA",  "ECA",  "SSA",  "AM", "AP")

cntryIncome <- c("LM", "L", "LM", "L", "LM", "LM", "H", "UM", "UM", "UM", "LM", "UM", "LM", "H",  
                 "LM", "LM", "H", "UM", "UM", "LM", "LM", "UM", "UM", "LM", "L", "L", "LM", "LM",
                 "L", "LM", "UM", "LM", "L", "UM", "UM", "L", "L", "L", "L", "H", "LM")
...
write.csv(dgroup, "CSV/dgroup.csv")
...
Urbanicity <- fct_collapse(as.factor(dgroup$Urbanicity), "Rural" = "1", "Urban" = "2", "Not known" = "98", "Refused" = 99)
dgroup$Urbanicity <- Urbanicity
dgroup <- dgroup %>% filter(Urbanicity != "Not known" & Urbanicity != "Refused")
...
write.csv(dgroup_pivot, "CSV/dqq Subgroups.csv", row.names = FALSE)
