...
start.time <- Sys.time()
d <- d %>% 
  mutate(MDD = coalesce(if_any(c(DQQ1, DQQ2, DQQ3), ~.x == 1) + 
                        if_any(c(DQQ14, DQQ15, DQQ25), ~.x == 1) +
                        if_any(c(DQQ16, DQQ17, DQQ18, DQQ19, DQQ20), ~.x == 1) +
                        if_any(c(DQQ6.1, DQQ6.2), ~.x == 1) +
                        if_any(c(DQQ5, DQQ8), ~.x == 1) +
                        if_any(c(DQQ7.1, DQQ7.2, DQQ7.3), ~.x == 1) +
                        if_any(c(DQQ9, DQQ10.1, DQQ10.2), ~.x == 1), 0))

end.time <- Sys.time()
time.taken <- end.time - start.time
time.taken # Time difference of 0.1409111 secs
write_csv(d, "MDD_Version_3.csv")
