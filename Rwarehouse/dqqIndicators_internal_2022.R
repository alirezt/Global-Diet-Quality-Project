...
d %>% filter(FieldDate == 2022) %>% group_by(Country) %>% summarise(n = n()) # 18 in 2022
d %>% filter(FieldDate == 2021) %>% group_by(Country) %>% summarise(n = n()) %>% print(n=50) # 38 in 2021
...
d$Residence <- ifelse(d$DEGURBA_2021_F2F == 2 | d$DEGURBA_2021_F2F == 3, "Urban",
                      ifelse(d$DEGURBA_2021_F2F == 1, "Rural", NA)) # levels swapped for 2021. remaining NAs: 36,559
...
# write_csv(d, "Data/Output/CSV/d_complete.csv")
...
head(results)
glimpse(results)
write_csv(results, "Data/Output/CSV/results_internal.csv")
