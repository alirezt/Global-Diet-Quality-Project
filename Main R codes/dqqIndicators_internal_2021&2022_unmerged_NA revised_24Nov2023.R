...
## 2.1 Adding start and end months ----

# Point: using "%m-%Y" cause max and min functions prioritize on months not year
d$FieldDate <- format(as.Date(d$FieldDate/86400, origin = "1582-10-14"), "%Y-%m-%d")
d$WP4 <- format(as.Date(d$WP4/86400, origin = "1582-10-14"), "%Y-%m-%d")

d <- d %>%
  group_by(Country, YEAR_WAVE) %>%
  mutate(
    Start_month = min(WP4),
    End_month = max(WP4)
  )
...
# extract months
d$Start_month <- month(d$Start_month)
d$End_month <- month(d$End_month)

#write_csv(d, "Data/Output/CSV/d_native_withDates.csv")
...
# Note: d was grouped before for adding start and end months
# it is necessary to make it a df before survey design
d <- data.frame(d)

# 5. Complex Survey design ----
result1 <- setNames(data.frame(matrix(ncol = 16, nrow = 1)), c("World_bank_income_group", "Region", "Country", "ISO3", "Subgroup", "Variable", "Year", "Start_month", "End_month", "Mean_Prevalence", "LCI", "UCI", "Difference", "Diff_LCI", "Diff_UCI", "Diff_p"))
result2 <- setNames(data.frame(matrix(ncol = 16, nrow = 1)), c("World_bank_income_group", "Region", "Country", "ISO3", "Subgroup", "Variable", "Year", "Start_month", "End_month", "Mean_Prevalence", "LCI", "UCI", "Difference", "Diff_LCI", "Diff_UCI", "Diff_p"))
...
write_csv(results, "Data/Output/CSV/results_internal_unmerged_NA_Revised_24Nov.csv")
