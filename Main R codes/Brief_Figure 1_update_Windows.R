...
# 2. Setting the font for Windows----
# run once 
...
df <- read_csv("Data/Output/CSV/results_public_merged.csv")
glimpse(df)
...
figure1 <- function(df, dfline, Country_name, year){
  df %>% filter(Country == Country_name & Year == year & Indicator %in% c("MDD-W",
    ...
    geom_text(aes(label = paste0(mean,"%"), y = mean - 1), color = "white", 
              size = 4.5, vjust = rep(c(0.5, 0.7, 0.5, 0.3), 5), hjust = 1, fontface = "bold.italic", family = "Whitney",   position = position_dodge(-.9), alpha = .9, show.legend = FALSE) +
    coord_flip(clip="off") + 
    ...
    geom_text(aes(label = "Not applicable", y = 0, x = 4.63), hjust = -.05, family = "Whitney", color = '#42225C', size = 4.5)+
    theme(
...
figure1_us <- figure1(df = df1, dfline = linesdata ,  Country_name = "United States 2022", year = 2022)
...
fgdsGraph <- function(Country_name, year){
  ...
  ggplot(fgds, aes(x = Indicator, y = mean_pre, fill = stack)) + 
    ...
    labs(x= "", y = "") +  
    annotate("text", x = 1.1, y = d$mean_pre[1] + .15, label = round(d$mean_pre[1], digits = 1), family = "Whitney", color = "#488135", fontface = "bold.italic", size = 4.5, hjust = 0) +
    annotate("text", x = 4, y = 0, label = "Food group diversity score (#/10)\n", size = 5, family =  "Whitney", fontface = "bold.italic", hjust = 0, lineheight = .95) +
    annotate("text", x = -2, y = 0, label = paste("\nUrban:", round(d$mean_pre[4], digits = 1), " ", "Rural:", round(d$mean_pre[5], digits = 1), " ", "Female:", round(d$mean_pre[3], digits = 1), " ", "Male:", round(d$mean_pre[2], digits = 1), sep = " "), 
             size = 4, family =  "Whitney", hjust = 0, lineheight = .95, fontface = "italic") + 
...
FGDS <- fgdsGraph("United States 2022", year = 2022)

# 6 Combine two graphs and save----
Brief <- plot_grid(figure1_us, FGDS, rel_heights = c(1, 0.1), nrow = 2, ncol = 1)
ggsave(file = "Data/Output/Graphs/United States 2022 Graph 1.pdf", 
