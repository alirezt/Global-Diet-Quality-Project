...
# check Whitney font family is set
"Whitney" %in% font_families()
...
figure2 <- function(df, dfline, Country_name, year){
  df %>%
    ...
    geom_text(aes(label = paste0(mean,"%"), y = mean - 1), color = "white", size = 4.5, 
              vjust = rep(c(0.5, 0.7, 0.5, 0.3), 5), hjust = 1, fontface = "bold.italic", 
              family = "Whitney", position = position_dodge(-0.9), alpha = .9, show.legend = FALSE) + 
    coord_flip(clip="off") +
...
ncdpGraph <- function(Country_name, year){
  ...
  ggplot(ncdp_bind, aes(x = Indicator, y = mean_pre, fill = stack)) + 
    ...
    annotate("text", x = 1.1, y = d$mean_pre[1] + .15, label = round(d$mean_pre[1], digits = 1), 
             family = "Whitney", color = "#488135", fontface = "bold.italic",  size = 4.5, hjust = 0) +
