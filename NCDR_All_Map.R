...
brk <- ni |>
  append(
    max(dat_ncdr$Mean_prevalence)
  ) |>
  head(-1)

breaks <- c(
  min(dat_ncdr$Mean_prevalence), brk
) |>
  tail(-1)
...
dat_ncdr %>%
  ...
  scale_fill_gradientn(name = "NCD-Risk",
                       colors = cols,
                       breaks = breaks,
                       labels = round(breaks, 1),
                       limits = c(
                         min(dat_ncdr$Mean_prevalence),
                         max(dat_ncdr$Mean_prevalence)
                       ),
  ...
  labs(x = NULL,
       y = NULL,
       title = "NCD-Risk",
       subtitle = "NCD-Risk values in subcategory <span style='color:#0072B2;'>**All**</span> 
       across the world for year 2022",
       caption = "<span style = 'font-size:25pt;'>The NCD-Risk score is a score with a range from 0 to 9. 
       A higher score indicates higher consumption of foods and drinks to avoid or limit, 
       and correlates negatively with meeting global dietary recommendations. 
       The NCD-Risk score is based on food consumption 
       from <span style = 'color:#0072B2;'>**8 food groups to limit**</span> or avoid during 
       the past day and night (one food group, processed meat, is double weighted)</span>."
