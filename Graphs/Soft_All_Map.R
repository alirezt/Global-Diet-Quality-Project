...
dat_softd <-  dat_agg %>%
  filter(Indicator == "Soft drink consumption" & Subgroup == "All") %>%
...
brk <- ni |>
  append(
    max(dat_softd$Mean_prevalence)
  ) |>
  head(-1)

breaks <- c(
  min(dat_softd$Mean_prevalence), brk
) |>
  tail(-1)
...
dat_softd %>%
  ...
  scale_fill_gradientn(name = "Soft drink consumption",
                       colors = cols,
                       breaks = breaks,
                       labels = round(breaks, 0),
                       limits = c(
                         min(dat_softd$Mean_prevalence),
                         max(dat_softd$Mean_prevalence)
                       ),
  ...
  labs(x = NULL,
       y = NULL,
       title = "Soft drink consumption",
       subtitle = "Soft drink consumption values in subcategory <span style='color:#8b0000;'>**All**</span> 
       across the world for year 2022",
       caption = "<span style = 'font-size:25pt;'>This indicator shows the proportion of the total population who 
       consumed a sugar-sweetened soft drink <span style = 'color:#8b0000;'>**(soda, energy drinks, or sports drinks)**</span> 
       in the previous day. 
       Soft drinks are strongly associated with excess sugar consumption and risk of NCDs, and are also 
       a marker of dietary and nutrition transitions.</span>"
