...
dat_dds <-  dat_agg %>%
  filter(Indicator == "Dietary diversity score" & Subgroup == "All") %>%
...
brk <- ni |>
  append(
    max(dat_dds$Mean_prevalence)
  ) |>
  head(-1)

breaks <- c(
  min(dat_dds$Mean_prevalence), brk
  ) |>
  tail(-1)
...
dat_dds %>%
  ...
  scale_fill_gradientn(name = "Dietary diversity score",
                       colors = cols,
                       breaks = breaks,
                       labels = round(breaks, 1),
                       limits = c(
                         min(dat_dds$Mean_prevalence),
                         max(dat_dds$Mean_prevalence)
                       ),
  ...
  labs(x = NULL,
       ...
       subtitle = "DDS values in subcategory <span style='color:#0072B2;'>**All**</span> 
       across the world for year 2022",
       caption = "<span style = 'font-size:25pt;'>The DDS is a semi-continuous score (0-10), 
       expressed as <span style = 'color:#0072B2;'>**the average score**</span> out of 10 for
       the population. A score of 5 or more indicates
       higher likelihood of adequate micronutrient intake.</span>"
       ) +
  theme_minimal() +
  theme(
